...
- Never hardcode key checks (e.g. `matchesKey(keyData, "ctrl+x")`). Add defaults to `TUI_KEYBINDINGS` (`packages/tui/src/keybindings.ts`) or `KEYBINDINGS` (`packages/coding-agent/src/core/keybindings.ts`) so they stay configurable.
