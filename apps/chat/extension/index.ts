...
import { Box, Text } from "@pie-lab/tui";
...
function createDashboardTmux(): string {
	...
	if (workers.length === 0) throw new Error("No managed pie-chat workers are running.");
...
function spawnConversationTmux(ctx: ExtensionContext, conversation: ResolvedConversation, restart: boolean): string {
	...
	session.appendSessionInfo(`pie-chat ${conversation.conversationName}`);
...
function createPieChatExtension(pi: ExtensionAPI) {
	pi.registerFlag(CHAT_CONVERSATION_FLAG, {
		description: "Auto-connect pie-chat to a configured account/channel",
		type: "string",
	});
	...
	let ownerId = `pie-chat-${process.pid}-${randomUUID()}`;
					...
					onMessage: async (input, checkpoint) => {
						...
						if (control === "new") {
							const queueNewSession = async () => {
								...
								await liveConnection?.sendImmediate("Starting a new pie session.");
							};
							if (chatTurnInFlight || !ctx.isIdle()) {
								pendingControlAction = queueNewSession;
								ctx.abort();
								await liveConnection?.sendImmediate("Aborting current turn, then starting a new pie session.");
								return;
							}
	...
	pi.registerMessageRenderer("chat-context", (message, _options, theme) => {
		...
		box.addChild(new Text(`${theme.fg("accent", theme.bold("[pie-chat]"))} ${String(message.content)}`, 0, 0));
		return box;
	});
	...
	pi.registerTool({
		name: "chat_workers",
		label: "Chat Workers",
		description: "Show configured pie-chat worker status from tmux and worker status snapshots.",
	...
	pi.registerTool({
		name: "chat_attach",
		label: "Chat Attach",
		description: "Queue one or more local files to be sent with the next pie-chat reply.",
	...
	pi.registerCommand("chat-config", {
		description: "Configure pie-chat Discord and Telegram accounts and channels",
	...
	pi.registerCommand("chat-spawn-all", {
		description: "Spawn all configured pie-chat channels in detached tmux sessions",
	...
	pi.registerCommand("chat-workers", {
		description: "Show managed pie-chat tmux sessions",
		handler: async (_args, ctx) => {
			await loadConfigOnce();
			ensureTmux();
			const config = await loadChatConfig();
	...
	pi.registerCommand("chat-open-all", {
		description: "Open all running pie-chat workers in a tiled tmux dashboard",
		handler: async (_args, ctx) => {
			await loadConfigOnce();
			ensureTmux();
			try {
	...
	pi.registerCommand("chat-kill-all", {
		description: "Kill all managed pie-chat tmux sessions",
		handler: async (_args, ctx) => {
			await loadConfigOnce();
			ensureTmux();
			const killed = killManagedTmuxSessions();
			ctx.ui.notify(
				killed.length > 0 ? `Killed:\n${killed.join("\n")}` : "No managed pie-chat tmux sessions running.",
				"info",
			);
		},
	});
	...
	pi.registerCommand("chat-connect", {
		description: "Connect this pie session to account/channel",
		handler: async (args, ctx) => {
			await loadConfigOnce();
			const config = await loadChatConfig();
			let spec = args.trim();
			if (!spec) {
				...
				spec = (await selectItem(ctx, "Connect pie-chat channel", items)) || "";
				if (!spec) return;
			}
	...
	pi.registerCommand("chat-new", {
		description: "Start a new pie session and keep the current pie-chat connection",
		handler: async (_args, ctx) => {
			const conversationId = runtime?.conversation.conversationId;
	...
	pi.registerCommand("chat-disconnect", {
		description: "Disconnect the current pie-chat channel",
		handler: async (_args, ctx) => {
	...
	pi.registerCommand("chat-status", {
		description: "Show pie-chat connection status",
		handler: async (_args, ctx) => {
	...
	pi.on("session_start", async (_event, ctx) => {
		await loadConfigOnce();
		ownerId = `pie-chat-${process.pid}-${randomUUID()}`;
	...
	pi.on("agent_end", async (event, ctx) => {
		...
		if (summary.stopReason === "error" || summary.stopReason === "length") {
			...
			if (liveConnection) {
				try {
					await liveConnection.sendImmediate(`pie-chat error: ${errorMessage}`);
				} catch {
...
export default createPieChatExtension;
