...
export const CHAT_HOME = join(homedir(), ".pie", "agent", "chat");
...
function buildResolvedConversation(
	...
	return {
		...
		botName: config.botName?.trim() || "pie",
		accountId,
		account,
		channelKey,
		channel,
...
export async function removeAccountStorage(accountId: string): Promise<void> {
...
export async function removeChannelStorage(accountId: string, channelKey: string): Promise<void> {
