import type { ExtensionContext } from "@pie-lab/coding-agent";
...
async function configureConfiguredChannel(
	...
	while (true) {
		...
		if (choice === "delete") {
			...
			await saveChatConfig(config);
			await removeChannelStorage(accountId, channelKey);
...
async function configureDiscordAccount(ctx: ExtensionContext, accountId: string): Promise<void> {
	while (true) {
		...
		if (choice === "delete") {
			...
			await saveChatConfig(config);
			await removeAccountStorage(accountId);
...
export async function runChatConfigUI(ctx: ExtensionContext): Promise<void> {
	...
	while (true) {
		...
		const choice = await selectItem(ctx, "pie-chat accounts", [
		...
		if (choice === "__secrets__") {
			await configureSecrets(ctx, "pie-chat secrets", config.gondolin, async (next) => {
