...
Stateful agent with tool execution and event streaming. Built on `@pie-lab/ai`.

## Installation

```bash
npm install @pie-lab/agent-core
```
...
```typescript
import { Agent, streamProxy } from "@pie-lab/agent-core";
...
```typescript
import { agentLoop, agentLoopContinue } from "@pie-lab/agent-core";
