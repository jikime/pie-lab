...
```bash
npm install @pie-lab/ai
```

TypeBox exports are re-exported from `@pie-lab/ai`: `Type`, `Static`, and `TSchema`.
...
```typescript
import { Type, type Context, type Tool } from '@pie-lab/ai';
import { builtinModels } from '@pie-lab/ai/providers/all';
...
```typescript
import { anthropicProvider } from '@pie-lab/ai/providers/anthropic';
import { openaiProvider } from '@pie-lab/ai/providers/openai';
import { openrouterProvider } from '@pie-lab/ai/providers/openrouter';
import { amazonBedrockProvider } from '@pie-lab/ai/providers/amazon-bedrock';
...
```typescript
import { getBuiltinModel, getBuiltinModels, getBuiltinProviders } from '@pie-lab/ai/providers/all';
...
```typescript
import { createModels, type CredentialStore } from '@pie-lab/ai';
...
```typescript
import { Type, type Tool, StringEnum } from '@pie-lab/ai';
...
```typescript
import { validateToolCall, type Tool } from '@pie-lab/ai';
...
```typescript
import { builtinImagesModels } from '@pie-lab/ai/providers/all';
...
```typescript
import { getImageModel, generateImages } from '@pie-lab/ai/compat';
...
```typescript
import { createModels, createProvider, envApiKeyAuth, type Model } from '@pie-lab/ai';
import { openAICompletionsApi } from '@pie-lab/ai/api/openai-completions.lazy';
...
```typescript
import { anthropicMessagesApi } from '@pie-lab/ai/api/anthropic-messages.lazy';
import { openAIResponsesApi } from '@pie-lab/ai/api/openai-responses.lazy';
...
```typescript
import { stream } from '@pie-lab/ai/api/anthropic-messages';
...
Importing an implementation module loads its SDK. The `./api/<id>.lazy` wrappers (used by the provider factories) defer that load to the first request when the runtime or bundler supports dynamic import chunking. Legacy raw API subpaths from older releases (`./anthropic`, `./google`, `./mistral`, `./openai-completions`, ...) were removed; use `@pie-lab/ai/api/<api-id>`.
...
```typescript
import { createModels, type Context } from '@pie-lab/ai';
import { anthropicProvider } from '@pie-lab/ai/providers/anthropic';
import { openaiProvider } from '@pie-lab/ai/providers/openai';
import { googleProvider } from '@pie-lab/ai/providers/google';
...
```typescript
import { createModels } from '@pie-lab/ai';
import { anthropicProvider } from '@pie-lab/ai/providers/anthropic';
...
- `@pie-lab/ai` is the core entrypoint and does not import built-in catalogs, provider factories, or SDK implementations.
- `@pie-lab/ai/providers/<provider>` imports that provider's catalog and lazy API wrapper only.
- `@pie-lab/ai/providers/all` imports every built-in provider factory and all catalogs. Use it only when you want the full built-in set.
...
- Importing `@pie-lab/ai/api/<api-id>` directly loads that API implementation and its SDK immediately.

Avoid `@pie-lab/ai/compat` in new bundled apps; it preserves the old global API and imports the full built-in catalog surface.
...
```typescript
import { setBedrockProviderModule } from '@pie-lab/ai/api/bedrock-converse-stream.lazy';
import { bedrockProviderModule } from '@pie-lab/ai/bedrock-provider';
...
```bash
npx @pie-lab/ai login              # interactive provider selection
npx @pie-lab/ai login anthropic    # login to specific provider
npx @pie-lab/ai list               # list available providers
```
...
The legacy flow functions remain available via the `@pie-lab/ai/oauth` entry point (`loginAnthropic`, `loginOpenAICodex`, `loginGitHubCopilot`, `refreshOAuthToken`, `getOAuthApiKey`); credential storage is the caller's responsibility there. New code should prefer the provider-owned `OAuthAuth` shown above — it composes with the credential store and gets locked auto-refresh for free.
...
```typescript
// Before
import { getModel, complete } from '@pie-lab/ai';

// After (verbatim behavior, one import-path change)
import { getModel, complete } from '@pie-lab/ai/compat';
```
...
| `streamAnthropic(model, ctx, opts)` | `stream` from `@pie-lab/ai/api/anthropic-messages`, or a provider in a collection |
...
Add a lazy wrapper `src/api/<api-id>.lazy.ts` (`<name>Api()` via `lazyApi()`) so providers can reference the implementation without importing its SDK. Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@pie-lab/ai`.
