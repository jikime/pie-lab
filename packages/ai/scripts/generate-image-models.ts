...
function generateImageModelsFile(models: ImagesModel<"openrouter-images">[]): string {
	const serializeStringArray = (values: readonly string[]) => `[${values.map((value) => JSON.stringify(value)).join(", ")}]`;
	const serializeCost = (cost: ImagesModel<"openrouter-images">["cost"]) => `{
				input: ${cost.input},
				output: ${cost.output},
