...
async function main(): Promise<void> {
	...
	if (!command || command === "help" || command === "--help" || command === "-h") {
		...
		console.log(`Usage: npx @pie-lab/ai <command> [provider]
...
Examples:
  ...
  npx @pie-lab/ai list               # list providers
`);
		return;
	}
	...
	if (command === "login") {
		...
		if (!PROVIDERS.some((p) => p.id === provider)) {
			...
			console.error(`Use 'npx @pie-lab/ai list' to see available providers`);
			process.exit(1);
		}
	...
	console.error(`Use 'npx @pie-lab/ai --help' for usage`);
	process.exit(1);
}
