...
The extension factory can also be `async`. For dynamic model discovery, fetch and register models in the factory instead of `session_start`. pi waits for the factory before startup continues, so the provider is available during interactive startup and to `pie --list-models`.
...
```typescript
import type { OAuthCredentials, OAuthLoginCallbacks } from "@pie-lab/ai";
