...
> **Placement for /reload:** Put extensions in `~/.pie/agent/extensions/` (global) or `.pie/extensions/` (project-local) for auto-discovery. Use `pie -e ./path.ts` only for quick tests. Extensions in auto-discovered locations can be hot-reloaded with `/reload`.
...
| Package | Purpose |
|---------|---------|
| `@pie-lab/coding-agent` | Extension types (`ExtensionAPI`, `ExtensionContext`, events) |
| `typebox` | Schema definitions for tool parameters |
| `@pie-lab/ai` | AI utilities (`StringEnum` for Google-compatible enums) |
| `@pie-lab/tui` | TUI components for custom rendering |
...
```typescript
import { isToolCallEventType } from "@pie-lab/coding-agent";
...
```typescript
import { isBashToolResult } from "@pie-lab/coding-agent";
...
```typescript
import { createLocalBashOperations } from "@pie-lab/coding-agent";
...
```typescript
import { SessionManager } from "@pie-lab/coding-agent";
...
```typescript
import { Type } from "typebox";
import { StringEnum } from "@pie-lab/ai";
...
```typescript
import type { AutocompleteItem } from "@pie-lab/tui";
...
If you need to discover models from a remote endpoint, prefer an async extension factory over deferring the fetch to `session_start`. pi waits for the factory before startup continues, so the registered models are available immediately, including to `pie --list-models`.
...
```typescript
import { withFileMutationQueue } from "@pie-lab/coding-agent";
...
**Important:** Use `StringEnum` from `@pie-lab/ai` for string enums. `Type.Union`/`Type.Literal` doesn't work with Google's API.
...
```typescript
import { createReadTool, createBashTool, type ReadOperations } from "@pie-lab/coding-agent";
...
```typescript
import { createBashTool } from "@pie-lab/coding-agent";
...
```typescript
import { keyHint } from "@pie-lab/coding-agent";
...
```typescript
import { Text, Component } from "@pie-lab/tui";
...
```typescript
import { CustomEditor, type ExtensionAPI } from "@pie-lab/coding-agent";
import { matchesKey } from "@pie-lab/tui";
...
```typescript
import { highlightCode, getLanguageFromPath } from "@pie-lab/coding-agent";
