...
`enableInstallTelemetry` only controls the anonymous install/update ping to `https://pielab.ai/api/report-install`. Opting out of telemetry does not disable update checks; pie can still fetch `https://pielab.ai/api/latest-version` to look for the latest version.

Set `PIE_SKIP_VERSION_CHECK=1` to disable the Pi version update check. Use `--offline` or `PIE_OFFLINE=1` to disable all startup network operations described here, including update checks, package update checks, and install/update telemetry.
...
When multiple sources specify a session directory, precedence is `--session-dir`, `PIE_CODING_AGENT_SESSION_DIR`, then `sessionDir` in settings.json.
...
Paths in `~/.pie/agent/settings.json` resolve relative to `~/.pie/agent`. Paths in `.pie/settings.json` resolve relative to `.pie`. Absolute paths and `~` are supported.
