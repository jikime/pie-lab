...
Pie binds `Ctrl+J` as a default newline alias, so `Shift+Enter` keeps working in tmux via that remap without extra pie configuration.
...
If you want the hardware cursor visible, set `PIE_HARDWARE_CURSOR=1` before running pi (disabled by default for compatibility).
