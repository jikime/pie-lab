...
**Source:** [`@pie-lab/tui`](https://github.com/earendil-works/pi-mono/tree/main/packages/tui)
...
```typescript
import { CURSOR_MARKER, type Component, type Focusable } from "@pie-lab/tui";
...
```typescript
import { Container, type Focusable, Input } from "@pie-lab/tui";
...
Import from `@pie-lab/tui`:

```typescript
import { Text, Box, Container, Spacer, Markdown } from "@pie-lab/tui";
```
...
```typescript
import { matchesKey, Key } from "@pie-lab/tui";
...
```typescript
import { visibleWidth, truncateToWidth } from "@pie-lab/tui";
...
```typescript
import { getMarkdownTheme } from "@pie-lab/coding-agent";
import { Markdown } from "@pie-lab/tui";
...
Set `PIE_TUI_WRITE_LOG` to capture the raw ANSI stream written to stdout.

```bash
PIE_TUI_WRITE_LOG=/tmp/tui-ansi.log npx tsx packages/tui/test/chat-simple.ts
```
...
For letting users pick from a list of options. Use `SelectList` from `@pie-lab/tui` with `DynamicBorder` for framing.

```typescript
import type { ExtensionAPI } from "@pie-lab/coding-agent";
import { DynamicBorder } from "@pie-lab/coding-agent";
import { Container, type SelectItem, SelectList, Text } from "@pie-lab/tui";
...
For toggling multiple settings. Use `SettingsList` from `@pie-lab/tui` with `getSettingsListTheme()`.

```typescript
import { getSettingsListTheme } from "@pie-lab/coding-agent";
import { Container, type SettingItem, SettingsList, Text } from "@pie-lab/tui";
...
import { matchesKey, truncateToWidth } from "@pie-lab/tui";
