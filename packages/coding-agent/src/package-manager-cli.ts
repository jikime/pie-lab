...
import { getLatestPieRelease, isNewerPackageVersion } from "./utils/version-check.ts";
import {
...
function getPackageCommandUsage(command: PackageCommand): string {
	switch (command) {
		case "install":
			return `${CLI_NAME} install <source> [-l] [--approve|--no-approve]`;
		case "remove":
			return `${CLI_NAME} remove <source> [-l] [--approve|--no-approve]`;
		case "update":
			return `${CLI_NAME} update [source|self|pie] [--self|--extensions|--all] [--extension <source>] [--approve|--no-approve] [--force]`;
		case "list":
			return `${CLI_NAME} list [--approve|--no-approve]`;
	}
}
...
Options:
  --self                  Update pie only (default when no target is given)
  --extensions            Update installed packages only
  --all                   Update pie and installed packages
...
Short forms:
  ${CLI_NAME} update                Update pie only
  ${CLI_NAME} update --all          Update pie and all extensions
...
async function getSelfUpdatePlan(force: boolean): Promise<SelfUpdatePlan> {
	let latestRelease: Awaited<ReturnType<typeof getLatestPieRelease>>;
	try {
		latestRelease = await getLatestPieRelease(VERSION);
	} catch (error: unknown) {
		...
		switch (options.command) {
			...
			case "update": {
				...
				if (updateTargetIncludesSelf(target)) {
					...
					console.log(chalk.green(`Updated ${CLI_NAME} from ${VERSION} to ${selfUpdatePlan.version}`));
				}
				return true;
			}
		}
