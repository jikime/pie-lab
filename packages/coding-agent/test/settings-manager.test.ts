...
describe("SettingsManager", () => {
	...
	describe("project settings directory creation", () => {
		it("should not create .pie folder when only reading project settings", () => {
			// Create agent dir with global settings, but NO .pie folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));

			// Delete the .pie folder that beforeEach created
			rmSync(join(projectDir, ".pie"), { recursive: true });

			// Create SettingsManager (reads both global and project settings)
			const manager = SettingsManager.create(projectDir, agentDir);

			// .pie folder should NOT have been created just from reading
		...
		it("should create .pie folder when writing project settings", async () => {
			// Create agent dir with global settings, but NO .pie folder in project
			const settingsPath = join(agentDir, "settings.json");
			writeFileSync(settingsPath, JSON.stringify({ theme: "dark" }));
			...
			// .pie folder should NOT exist yet
			expect(existsSync(join(projectDir, ".pie"))).toBe(false);
