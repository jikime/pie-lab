...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".pie", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".pie", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".pie", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".pie"), { recursive: true });
			writeFileSync(join(cwd, ".pie", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(cwd)).toBe(true);

			rmSync(join(cwd, ".pie"), { recursive: true, force: true });
