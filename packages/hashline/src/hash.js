 ...
 */
export function computeHashContext(lines, lineIndex, contextLines = 2) {
 ...
 */
export function isLikelyMatch(currentLine, targetAnchor, currentHash, targetHash) {
    // Exact match
    if (currentLine === targetAnchor)
        return true;
    // Hash match (if both provided)
    if (currentHash && targetHash && currentHash === targetHash)
        return true;
    // Fuzzy match: first 50 chars match
    if (currentLine.substring(0, 50) === targetAnchor.substring(0, 50))
        return true;
    return false;
}
/**
 ...
 */
export function findAnchorLine(lines, anchor, targetHash, nearLine) {
    // 1. Exact match
