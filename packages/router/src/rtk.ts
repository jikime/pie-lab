...
export function compressPayloadWithRtk(payload: unknown, enabled = true): RtkPayloadResult {
...
export function compressMessages(body: unknown, enabled = true): RtkStats | null {
	...
	for (const item of items) {
		...
		for (const block of message.content) {
			...
			if (blockRecord.type !== "tool_result" || blockRecord.is_error === true) continue;
...
function compressKiroFormat(body: Record<string, unknown>): RtkStats {
	const stats: RtkStats = { bytesBefore: 0, bytesAfter: 0, hits: [] };
	...
	for (const message of messages) {
		...
		for (const toolResult of toolResults) {
			...
			if (resultRecord.status === "error" || !Array.isArray(resultRecord.content)) continue;
			for (const part of resultRecord.content) {
				if (part && typeof part === "object" && typeof (part as { text?: unknown }).text === "string") {
					(part as { text: string }).text = compressText(
...
function autoDetectFilter(text: string): TextFilter | null {
	...
	if (/^diff --git /m.test(sample)) return withName(compactGitDiff, "gitDiff");
	if (/^(On branch|Changes not staged|Untracked files:|[ MADRCU?!]{2}\s+\S+)/m.test(sample)) {
		return withName(compactGitStatus, "gitStatus");
	}
	if (/^.+:\d+(:\d+)?:/m.test(sample)) return withName(compactSearchList, "searchList");
...
function compactGitStatus(input: string): string {
	...
	if (porcelain.length === 0) return smartTruncate(input);
	...
	return [...groups.entries()]
		.map(
			([status, files]) =>
				`${status}: ${files.slice(0, 80).join(", ")}${files.length > 80 ? ` ... +${files.length - 80}` : ""}`,
		)
		.join("\n");
}

function compactSearchList(input: string): string {
	const lines = input.split(/\r?\n/).filter(Boolean);
...
function smartTruncate(input: string): string {
	const lines = input.split(/\r?\n/);
	if (lines.length <= 160 && input.length <= 12_000) return input;
	const head = lines.slice(0, 80).join("\n");
	const tail = lines.slice(-40).join("\n");
	return `${head}\n\n[rtk truncated ${Math.max(0, lines.length - 120)} middle lines]\n\n${tail}`;
}

function safeApply(filter: TextFilter, input: string): string {
	try {
		return filter(input);
	} catch {
		return input;
...
function withName(filter: (input: string) => string, name: string): TextFilter {
	const named = filter as TextFilter;
...
function isTextPart(value: unknown, type: string): value is { type: string; text: string } {
	return (
		!!value &&
		typeof value === "object" &&
