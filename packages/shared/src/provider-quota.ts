...
import { PIE_LAB_QUOTA_SELECTION_KEY, type ProviderQuotaSelectionSnapshot } from "@pie-lab/router";
import {
...
export const USAGE_APIKEY_PROVIDERS = ["glm", "glm-cn", "minimax", "minimax-cn"] as const;
...
const PROXY_DISPATCHERS = new Map<string, unknown>();
...
export function createProviderQuotaRequestHandler(options: ProviderQuotaApiOptions = {}) {
	...
	return async (request: IncomingMessage, response: ServerResponse) => {
		try {
			await handleProviderQuotaRequest(
				request,
				response,
...
export function getDefaultProviderConnectionFilePath(agentDir = getDefaultAgentDir()): string {
	return join(agentDir, "provider-connections.json");
}

export async function handleProviderQuotaRequest(
	request: IncomingMessage,
	response: ServerResponse,
	providerConnectionStore: ProviderConnectionStore,
	proxyPoolStore: ProxyPoolStore = providerConnectionStore,
	...
	if (connectionId) {
		...
		if (isUsageEligibleConnection(connection)) {
			...
			if (isOAuthConnection(quotaConnection) && isAuthExpiredMessage(usage) && quotaConnection.refreshToken) {
				quotaConnection = await refreshAndUpdateCredentials(
					quotaConnection,
					providerConnectionStore,
					quotaFetch,
					true,
		...
		writeJson(response, 200, {
			...
			usage,
		} satisfies ProviderQuotaDetailResponse);
		return;
	}
...
function isQuotaListPath(pathname: string): boolean {
	return pathname === "/quota" || pathname === "/v1/quota";
}

function parseQuotaConnectionId(pathname: string): string | undefined {
	const match = pathname.match(/^\/(?:v1\/)?quota\/([^/]+)$/);
	return match ? decodeURIComponent(match[1]) : undefined;
}

function toProviderQuotaConnectionStatus(connection: ProviderConnection): ProviderQuotaConnectionStatus {
...
async function updateProviderQuotaSelectionSnapshot(
	providerConnectionStore: ProviderConnectionStore,
	connection: ProviderConnection,
	usage: ProviderUsageResult,
...
export function createProviderQuotaSelectionSnapshot(
	usage: ProviderUsageResult,
	now: Date = new Date(),
): ProviderQuotaSelectionSnapshot {
	const quotaWindows = Object.values(usage.quotas ?? {});
	const quotaScores = quotaWindows.map(quotaWindowToScore).filter((score): score is number => score !== null);
...
function quotaWindowToScore(quota: ProviderQuotaWindow): number | null {
...
function readQuotaSelectionSnapshot(connection: ProviderConnection): ProviderQuotaSelectionSnapshot | null {
	const snapshot =
		connection.providerSpecificData?.[PIE_LAB_QUOTA_SELECTION_KEY] ??
...
export function createQuotaAwareProviderConnectionPreparer(options: QuotaAwareProviderConnectionPreparerOptions) {
	...
	return async (input: {
		...
		const shouldRefresh =
			providerOverride.quotaRefreshBeforeSelection ?? input.settings.quotaRefreshBeforeSelection ?? true;
		if (quotaStrategy === "off" || !shouldRefresh) {
			return input.connections;
		}

		const refreshTtlMs = readPositiveNumberSetting(
			providerOverride.quotaRefreshTtlMs ?? input.settings.quotaRefreshTtlMs,
			60 * 1000,
		);

		await Promise.all(
			input.connections.map((connection) =>
				refreshProviderQuotaSelectionSnapshotIfNeeded({
					connection,
...
async function refreshProviderQuotaSelectionSnapshotIfNeeded(options: {
	...
	try {
		const proxyOptions = await resolveConnectionProxyOptions(
			options.connection.providerSpecificData,
			options.proxyPoolStore,
		);
		const quotaFetch = createProxyAwareFetch(options.fetchImpl, proxyOptions);
		let quotaConnection = await refreshAndUpdateCredentials(
			options.connection,
			options.providerConnectionStore,
			quotaFetch,
		);
		...
		const snapshot: ProviderQuotaSelectionSnapshot = {
...
function readPositiveNumberSetting(value: unknown, fallback: number): number {
...
function isUsageEligibleConnection(connection: ProviderConnection): boolean {
	return isUsageSupportedProvider(connection.provider) && getUsageAuthType(connection) !== "unsupported";
}

function isOAuthConnection(connection: ProviderConnection): boolean {
	return connection.authType === "oauth";
}

function isUsageSupportedProvider(provider: string): boolean {
	return (USAGE_SUPPORTED_PROVIDERS as readonly string[]).includes(provider);
}

function getUsageAuthType(connection: ProviderConnection): "oauth" | "apikey" | "unsupported" {
	if (connection.authType === "oauth") return "oauth";
	if (
		connection.authType === "apikey" &&
		(USAGE_APIKEY_PROVIDERS as readonly string[]).includes(connection.provider)
	) {
		return "apikey";
	}
	return "unsupported";
...
async function resolveConnectionProxyOptions(
...
function normalizeLegacyProxyOptions(providerSpecificData: Record<string, unknown> = {}): ProxyOptions {
	return {
...
function createProxyAwareFetch(fetchImpl: FetchLike, proxyOptions: ProxyOptions | null): FetchLike {
	return ((url: Parameters<FetchLike>[0], options?: Parameters<FetchLike>[1]) =>
		proxyAwareFetch(fetchImpl, url, options ?? {}, proxyOptions)) as FetchLike;
}
		...
		if (proxyUrl) {
			try {
				...
				if (proxyOptions?.strictProxy === true) {
					throw new Error(
						`[ProxyFetch] Proxy required but failed (strictProxy=true): ${formatErrorMessage(proxyError)}`,
					);
				}
			}
		}
...
function shouldBypassMitmDns(targetUrl: string): boolean {
	try {
...
async function getProxyDispatcher(proxyUrl: string): Promise<unknown> {
	...
	if (!PROXY_DISPATCHERS.has(normalized)) {
		if (PROXY_DISPATCHERS.size >= PROXY_DISPATCHERS_MAX_SIZE) {
			...
			if (firstKey) PROXY_DISPATCHERS.delete(firstKey);
		}
...
function fetchWithDispatcher(
	...
	return fetchImpl(url, { ...options, dispatcher } as RequestInit);
}
...
function createBypassRequest(parsedUrl: URL, realIp: string, options: RequestInit): Promise<Response> {
	return new Promise((resolve, reject) => {
		...
		socket.connect(HTTPS_PORT, realIp, () => {
			const req = httpsRequest(
				...
				} as unknown as Parameters<typeof httpsRequest>[0] & { socket: Socket },
				(res) => {
...
function serializeRequestBody(body: RequestInit["body"] | null | undefined): string | Buffer | undefined {
...
async function refreshAndUpdateCredentials(
	...
	if (!isOAuthConnection(connection)) return connection;
	if (!force && !needsCredentialRefresh(connection)) return connection;
	...
	const updateData: Parameters<ProviderConnectionStore["updateProviderConnection"]>[1] = {
...
function needsCredentialRefresh(connection: ProviderConnection): boolean {
	if (connection.provider === "github") {
		const providerSpecificData = connection.providerSpecificData ?? {};
		if (!providerSpecificData.copilotToken) return true;
		const copilotExpiresAt = getExpiryMs(providerSpecificData.copilotTokenExpiresAt);
		if (copilotExpiresAt !== null && copilotExpiresAt - Date.now() < TOKEN_REFRESH_LEAD_MS) return true;
	}
...
async function refreshProviderCredentials(
	...
	if (!connection.refreshToken && connection.provider !== "github") return null;

	try {
		switch (connection.provider) {
			case "claude":
				return refreshWithJson(
					fetchImpl,
					OAUTH_ENDPOINTS.anthropic,
					{
			...
			case "codex":
				return refreshWithForm(
					fetchImpl,
					OAUTH_ENDPOINTS.openai,
					{
						grant_type: "refresh_token",
						refresh_token: connection.refreshToken,
...
async function refreshWithJson(
	fetchImpl: FetchLike,
	url: string,
...
async function refreshWithForm(
	fetchImpl: FetchLike,
	url: string,
...
function normalizeOAuthTokenResponse(
	...
	return {
		accessToken,
		refreshToken:
			readString(tokens.refresh_token) ?? readString(tokens.refreshToken) ?? fallbackRefreshToken ?? undefined,
...
async function refreshGitHubCredentials(
	...
	if (!copilotResult && refreshToken) {
		const githubTokens = await refreshWithForm(
			fetchImpl,
...
): Promise<Pick<ProviderRefreshResult, "copilotToken" | "copilotTokenExpiresAt"> | null> {
...
function refreshKimiCodingToken(
	fetchImpl: FetchLike,
	refreshToken: string | null | undefined,
): Promise<ProviderRefreshResult | null> {
	if (!refreshToken) return Promise.resolve(null);
...
function isAuthExpiredMessage(usage: ProviderUsageResult): boolean {
...
function formatGitHubQuotaSnapshot(quotaValue: unknown): ProviderQuotaWindow {
	const quota = asRecord(quotaValue);
	const entitlement = readNumber(quota.entitlement);
	const remaining = readNumber(quota.remaining);
	if (Object.keys(quota).length === 0) return { used: 0, total: 0, unlimited: true };
...
async function getClaudeUsage(
	...
	const data = (await response.json()) as Record<string, unknown>;
	const quotas: Record<string, ProviderQuotaWindow> = {};

	const maybeAddWindow = (key: string, label: string) => {
...
async function getGeminiUsage(
	...
	try {
		...
		for (const bucketValue of buckets) {
			...
			if (!modelId || bucket.remainingFraction === undefined || bucket.remainingFraction === null) continue;
...
async function getGeminiSubscriptionInfo(
	accessToken: string,
	fetchImpl: FetchLike,
...
async function getAntigravityUsage(
	...
	try {
		...
		for (const [modelKey, infoValue] of Object.entries(models)) {
			...
			if (Object.keys(quotaInfo).length === 0) continue;
			if (info.isInternal === true || !ANTIGRAVITY_IMPORTANT_MODELS.has(modelKey)) continue;
...
function formatCodexWindow(windowValue: unknown): ProviderQuotaWindow {
...
function appendCodexQuotaWindows(
	quotas: Record<string, ProviderQuotaWindow>,
	prefix: string,
	snapshot: unknown,
): boolean {
	...
	const primary =
		rateLimit.primary_window ?? rateLimit.primary ?? asRecord(snapshot).primary_window ?? asRecord(snapshot).primary;
	const secondary =
	...
	const attempts: Array<{ name: string; run: () => Promise<Response> }> = [
		{
			...
			run: () =>
				fetchImpl(`https://codewhisperer.us-east-1.amazonaws.com/getUsageLimits?${getUsageParams.toString()}`, {
					method: "GET",
					headers: {
						Authorization: `Bearer ${accessToken}`,
						Accept: "application/json",
		...
		},
		{
			...
			run: () =>
				fetchImpl("https://codewhisperer.us-east-1.amazonaws.com", {
					method: "POST",
					headers: {
						Authorization: `Bearer ${accessToken}`,
	...
	for (const attempt of attempts) {
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 401 || response.status === 403) sawAuthError = true;
...
function parseKiroQuotaData(data: Record<string, unknown>): ProviderUsageResult {
	...
	for (const breakdownValue of usageList) {
		...
		if (Object.keys(freeTrialInfo).length === 0) continue;
...
function addMiniMaxQuota(
	...
	key: string,
	model: Record<string, unknown>,
	getTotal: (model: Record<string, unknown>) => number,
	countSnake: string,
	countCamel: string,
