...
const packages = [
	{ directory: "packages/ai", name: "@pie-lab/ai" },
	{ directory: "packages/tui", name: "@pie-lab/tui" },
	{ directory: "packages/agent", name: "@pie-lab/agent-core" },
	{ directory: "packages/router", name: "@pie-lab/router" },
	{ directory: "packages/storage", name: "@pie-lab/storage" },
	{ directory: "packages/shared", name: "@pie-lab/shared" },
	{ directory: "packages/coding-agent", name: "@pie-lab/coding-agent" },
];
...
function prepareOutputDirectory(options, repoRoot) {
	if (!options.outDir) {
		return mkdtempSync(join(tmpdir(), "pie-local-release-"));
	}
...
function buildBunBinaryRelease(targetDirectory, archiveDirectory) {
	...
	const archiveName = platform.startsWith("windows-") ? `pie-${platform}.zip` : `pie-${platform}.tar.gz`;
	cpSync(join(binaryBuildDirectory, archiveName), join(archiveDirectory, archiveName));
	return platform;
}

function createPieShim(installDirectory) {
	const binDirectory = join(installDirectory, "node_modules", ".bin");
	if (process.platform === "win32") {
		if (existsSync(join(binDirectory, "pie.cmd"))) {
			writeFileSync(join(installDirectory, "pie.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pie.cmd" %*\r\n');
			writeFileSync(join(installDirectory, "pie.ps1"), '& "$PSScriptRoot/node_modules/.bin/pie.ps1" @args\n');
			return;
		}
		writeFileSync(join(installDirectory, "pie.cmd"), '@ECHO off\r\n"%~dp0node_modules\\.bin\\pie.exe" %*\r\n');
		writeFileSync(join(installDirectory, "pie.ps1"), '& "$PSScriptRoot/node_modules/.bin/pie.exe" @args\n');
		return;
	}
	symlinkSync(join("node_modules", ".bin", "pie"), join(installDirectory, "pie"));
}
...
if (rootPackageJson.name !== "pie-lab") {
...
if (!options.skipInstall) {
	...
	createPieShim(nodeInstallDirectory);
	...
	if (!options.skipBunInstall) {
		...
		createPieShim(bunInstallDirectory);
	}
}
...
if (!options.skipInstall) {
	...
	console.log(`  ${join(outDir, `pie-${binaryPlatform}.${String(binaryPlatform).startsWith("windows-") ? "zip" : "tar.gz"}`)}`);
	console.log("\nRun the local Bun binary release from outside the repository:");
	console.log(`  ${join(binaryDirectory, String(binaryPlatform).startsWith("windows-") ? "pie.exe" : "pie")} --help`);

	console.log("\nIsolated npm install:");
	console.log(`  ${nodeInstallDirectory}`);
	console.log("\nRun the locally packed npm CLI from outside the repository:");
	console.log(`  ${join(nodeInstallDirectory, process.platform === "win32" ? "pie.cmd" : "pie")} --help`);
	...
	if (!options.skipBunInstall) {
		...
		console.log(`  ${join(bunInstallDirectory, process.platform === "win32" ? "pie.cmd" : "pie")} --help`);
	}
}
